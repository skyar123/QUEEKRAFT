...
export function generateHubMap(game) {
    game.depth = 0;
    game.inHub = true;
    // Reflect persistent progress in the HUD while in the village.
    game.zines = Object.keys((game.persistent && game.persistent.seenZines) || {}).length;
    game.historicalFigures = Object.keys((game.persistent && game.persistent.seenFigures) || {}).length;
    game.treasures = (game.persistent && game.persistent.treasures) || 0;
    game.map = {};
    game.items = [];
    game.npcs = [];
    game.trolls = [];
    ...
    // Return village stats so the caller can show a "village is growing" hint.
    return {
        residents: game.npcs.length,
        figuresMet,
        totalFigures: Object.keys(HISTORICAL_FIGURES).length,
        zinesCollected
    };
}
